...
export function MediaCard({
  ...
  return (
    <Drawer>
        ...
        <Link href={detailUrl} className="block w-full">
          <div className="relative w-full aspect-[16/12]">
            ...
            <div className="absolute bottom-4 left-4 text-white flex flex-col gap-1">
              ...
              {genre && (
                <span className="text-sm opacity-90 bg-black/60 rounded px-2 py-0.5 w-fit">
                  {genre}
                </span>
              )}
            </div>
            ...
            <span
              className={`absolute bottom-4 right-4 px-3 py-1 rounded-full text-xs font-bold shadow-lg ${typeBadge.className}`}
            >
              {typeBadge.label}
            </span>
          </div>
      ...
      <DrawerContent>
        <div className="w-full py-2 max-w-4xl mx-auto flex flex-col items-center">
          <div className="flex items-center gap-4">
            <h2 className="text-5xl font-extrabold tracking-tight drop-shadow-lg mb-1">
              {title}
            </h2>
            <span
              className={`px-4 py-1 rounded-full text-lg font-bold shadow-lg ${typeBadge.className}`}
            >
              {typeBadge.label}
            </span>
          ...
          {genre && (
            <span className="inline-block text-xl font-semibold bg-black/60 rounded px-4 py-1 mr-2 mb-2 w-fit">
              {genre}
            </span>
          )}
          ...
          <div className="w-full px-12 py-12 flex flex-col gap-12 items-start">
            ...
            {additionalInfo && (
              <div>
                ...
                <p className="text-4xl font-extrabold text-primary drop-shadow">
        ...
        <DrawerFooter className="flex justify-end w-full max-w-4xl mx-auto pb-8">
          <DrawerClose asChild>
            <Button className="text-lg px-8 py-3  ">Close</Button>
          </DrawerClose>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
